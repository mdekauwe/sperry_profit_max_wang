...
class ProfitMax(object):
    ...
    def __init__(self, sw0=0.5, psi_e=-0.8*cons.KPA_2_MPA, theta_sat=0.5, b=6.,
        ...
        self.vcmax = vcmax # per leaf area
        self.jmax = jmax   # per leaf area
    ...
    def sperry_optimisation(self, psi_soil, vpd, ca, tair, par, press):
        ...
        for i in range(101):
            ...
            # 0.01 is equallyy partioning range between 0 to e_crit
            e = i * 0.01 * e_crit
            ...
            #f   = e * 1000.0 / 18.0 / 1800.0
            #gh  = f / self.laba / vpd * 100.0
            #gc  = gh / 1.6
            #g   = gc * 10.0
    ...
    def update_sw_bucket(self, precip, water_loss, sw_prev):
        ...
        water_loss : float
            flux of water out of the soil (transpiration (mmol m-2 s-1))
        sw_prev : float
...
if __name__ == "__main__":
    ...
    lat = -35.76
    lon = 148.0
    met = generate_met_data(Tmin=10, Tmax=30.0, RH=30, ndays=100,
